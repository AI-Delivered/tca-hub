...
// The key itself is built by answerCacheKey in @/lib/query-key — shared, because
// triage has to reconstruct it to drop an answer found to still carry a fault.
...
export async function POST(req: NextRequest) {
  ...
  const cacheKey = answerCacheKey(query, now.toISOString().slice(0, 10))
